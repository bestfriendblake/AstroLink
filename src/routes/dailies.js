...
router.get('/status', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const lastClaimDate = streak?.last_claim_date
            ? new Date(streak.last_claim_date).toLocaleDateString('en-CA')
            : null;
        const canClaimStreak = !streak || lastClaimDate !== today;
...
router.post('/streak/claim', requireAuth, async (req, res) => {
    ...
    try {
        ...
        // Convert whatever MySQL gives us to a clean YYYY-MM-DD string
        ...
        // Check if yesterday to determine consecutive
        const yesterday = new Date();
        yesterday.setDate(yesterday.getDate() - 1);
        const yStr = yesterday.toLocaleDateString('en-CA');

        const isConsecutive = lastClaim === yStr;
